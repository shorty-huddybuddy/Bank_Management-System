...
public class Database {
	...
	void printAccounts() {
		for (int i = 0; i < account.size(); i++) {
			System.out.println(account.get(i).getAccuntNo() + " " +
					account.get(i).getPIN());
			...
			while ((type = br.readLine()) != null) {
				...
				if (type.equals(String.valueOf(Account.SAVINGS_ACCOUNT))) {
					ac = new SavingsAccount(br.readLine(), br.readLine(), Double.parseDouble(br.readLine()),
							new UserInformation(br.readLine(), br.readLine()));


							// new UserInformation(br.readLine(), br.readLine(), br.readLine(), br.readLine(),
							// 		br.readLine(),
